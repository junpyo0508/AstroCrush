...
const Starfield = () => {
  ...
  const init = (settings) => {
    ...
    const light = new THREE.PointLight(0xffffff, 0.05, 5);
    ...
    loader.load(modelPaths[7], (gltf) => {
      ...
      for (let i = 0; i < numBoxes; i += 1) {
        ...
        rock.rotation.set(rote.x, rote.y, rote.z);
        rock.scale.set(size, size, size);
    
        const hitBoxGeo = new THREE.BoxGeometry(size, size, size);
        ...
        hitBox.position.copy(rock.position);
        hitBox.name = "hitbox";
    
        const rockGroup = new THREE.Group();
        rockGroup.add(rock);
        rockGroup.add(hitBox);
    
        boxGroup.add(rockGroup);
      }
    }, undefined, (error) => {
      console.error("An error happened", error);
    });
    ...
    const getLaserBolt = () => {
      ...
      if (intersects.length > 0) {
        ...
        if (intersects[0].object.name === 'Object_2') {
          const hitObject = intersects[0].object;
          const parentGroup = hitObject.parent;
      
          console.log("Before remove:", boxGroup.children.length);
          if (parentGroup) {
              boxGroup.remove(parentGroup);
          }
          scene.remove(hitObject);
          if (hitObject.userData.rock) {
              scene.remove(hitObject.userData.rock);
          }
          console.log("After remove:", boxGroup.children.length);
